...
namespace CasualtiesUnknown.SaveManager
{
    ...
    ///
    /// KrokMP 主机每次世界生成都用 30010 包（Server_AnnounceSeed）把 LastBeforeGenerationState 广播给客户端，
    /// 客户端据此在本地重生成世界。本类：
    ///   · 主机（self 引擎）接管 Server_AnnounceSeed，在原结构体之后向同一个包追加 1 个 int（CurrentSeed）后发出；
    ///   · 客户端后置 ClientReciever__GenerateWorld_SeedAnnounce，读出尾部那个 int 并激活逐阶段重置。
    ...
    internal static class MpSeedBroadcast
    {
        ...
        private static MethodInfo _mSendToClients; // Net.Server_SendToClients(in DeliveryMethod, in NetDataWriter, in IReadOnlyList<uint>)
        ...
        internal static void TryPatch()
        {
            ...
            try
            {
                ...
                if (tNet == null || tExt == null || tServer == null || tClient == null
                    || tGenPatch == null || tState == null || tWriter == null || tReader == null || tDelivery == null)
                    return;
                ...
                _mSendToClients = tNet.GetMethods(BindingFlags.Public | BindingFlags.Static)
                    .FirstOrDefault(m =>
                    {
                        ...
                        return elem == typeof(IReadOnlyList<uint>);
                    });
                ...
                MethodInfo receiver = AccessTools.Method(tClient, "ClientReciever__GenerateWorld_SeedAnnounce");

                if (_fFirstParams == null || _mCreateWriter == null || _mPutStruct == null || _mWriterPutInt == null
                    || _mSendToClients == null || _deliveryReliableOrdered == null
        ...
        /// </summary>
        private static bool AnnounceSeed_Prefix(IReadOnlyList<uint> to_who)
        {
            ...
            try
            {
                ...
                object writer = _mCreateWriter.Invoke(null, new object[] { (ushort)30010 });
                ...
                ModLog.Info($"多人种子广播：随 30010 追加 saveManagerSeed={seed}");
                return false; // 已接管发送
            }
            catch (Exception ex)
            {
        ...
        /// <summary>客户端：30010 收完结构体后，若包尾还有 int 则读出；非 0 即激活逐阶段重置使世界与主机/同种子单机一致。</summary>
